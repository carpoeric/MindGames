...
public class Main
{
    ...
    {
        com.mindgames.GameApp.mainMenu();
        ...
        while (scanInput.equals("1")) {
            String numberInput;
            String[] numberArray = com.mindgames.GameApp.randomNumber();
            List<String> numberList = Arrays.asList(numberArray);
            Random random = new Random();
            String randomNumber = numberList.get(random.nextInt(numberList.size()));
            ...
            while (tries <= 10) {
                ...
                while ((numberInput.length() != 4) || (!numberList.contains(numberInput)))
                {
